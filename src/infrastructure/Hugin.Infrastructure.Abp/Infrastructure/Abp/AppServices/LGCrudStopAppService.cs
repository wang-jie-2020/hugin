...
namespace LG.NetCore.Infrastructure.Abp.AppServices
{
    ...
    public abstract class LGCrudStopAppService<TEntity, TKey, TEntityDto, TGetListInput, TEntityEditDto, TGetEditOutputDto, TCreateOrUpdateInput>
    ...
    where TGetEditOutputDto : new()
    {
        ...
        protected override IQueryable<T> ApplyFiltering<T>(IQueryable<T> query, TGetListInput input)
        {
            ...
            if (input is StopFilterPagedAndSortedResultRequestDto requestDto)
            {
                ...
                {
                    if (typeof(T).IsAssignableTo<IStopAudited>())
                    {
                        query = query.Where(p => ((IStopAudited)p).IsStop.Equals(requestDto.IsStop.Value));
                    }
                }
            }
