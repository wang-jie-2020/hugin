...
using LG.NetCore.Infrastructure.Abp.Dtos;
using LG.NetCore.Infrastructure.Attributes;
            ...
            where TGetEditOutputDto : new()
    {
        ...
        public virtual async Task<TGetEditOutputDto> GetForEdit(TKey? key)
        {
            ...
            if (key.HasValue)
            {
                ...
                if (output is IHoused<TEntityEditDto> housedOutput)
                {
        ...
        {
            TEntity entity;
            if (input is IHoused<TEntityEditDto> housedInput)
            {
        ...
        {
            if (entity is IUser user && HasUserIdProperty(entity) && user.UserId == Guid.Empty)
            {
                ...
                var propertyInfo = entity.GetType().GetProperty(nameof(IUser.UserId));
